...
# Sort models by Joint ALL 5-Fold CV Loss (Ascending Order: Best to Worst)
df = df.sort_values('Joint_ALL').reset_index(drop=True)
...
df['RPS_fast_norm'] = min_max_norm(df['RPS_fast'].values)
df['RPS_slow_norm'] = min_max_norm(df['RPS_slow'].values)
df['ESD_fast_norm'] = min_max_norm(df['ESD_fast'].values)
df['Fast+ESD_norm'] = min_max_norm(df['Fast+ESD'].values)
df['Joint_ALL_norm'] = min_max_norm(df['Joint_ALL'].values)

# 2. Build Single-Panel Normalized 5-Fold CV Plot
...
# Define metric lines & styles (All 5 metrics on left Y-axis)
METRICS_CONFIG = [
    {'col': 'Joint_ALL_norm', 'label': 'Joint ALL (5-Fold CV)',       'color': '#9467bd', 'marker': 'p', 'linestyle': '-',  'linewidth': 2.8, 'markersize': 8.5},
    {'col': 'Fast+ESD_norm',  'label': 'Fast+ESD Combined (5-Fold CV)', 'color': '#2ca02c', 'marker': 's', 'linestyle': '--', 'linewidth': 2.4, 'markersize': 7.5},
    {'col': 'ESD_fast_norm',  'label': 'ESD Scoreline (5-Fold CV)',  'color': '#e377c2', 'marker': 'D', 'linestyle': ':',  'linewidth': 2.2, 'markersize': 6.5},
    {'col': 'RPS_fast_norm',  'label': 'RPS Fast (5-Fold CV)',       'color': '#1f77b4', 'marker': 'o', 'linestyle': '-',  'linewidth': 2.0, 'markersize': 6.5},
    {'col': 'RPS_slow_norm',  'label': 'RPS Slow (5-Fold CV)',       'color': '#ff7f0e', 'marker': '^', 'linestyle': '-.', 'linewidth': 2.0, 'markersize': 6.5}
]
...
# Highlight Best Point (3-Elo Complete at x=0, y=0.0)
ax.plot(0, 0.0, '*', color='#d62728', markersize=18, zorder=10, label='3-Elo Complete (BEST)')

# Annotate Best Model Point
ax.annotate("BEST 5-Fold CV Performance\n3-Elo Complete (Norm Loss = 0.0)", xy=(0, 0.0),
             xytext=(0.4, 0.12),
...
ax.set_ylabel("Normalized Loss Value [0 = Best / Min, 1 = Worst / Max]", fontsize=12, fontweight='bold', color='#1D3557', labelpad=10)

ax.set_xticks(x_indices)
ax.set_xticklabels(model_labels, rotation=25, ha='right', fontsize=11, fontweight='bold', color='#1D3557')
ax.set_xlabel("Rating System Architectures (Sorted by Joint ALL 5-Fold CV Loss →)", fontsize=12, fontweight='bold', color='#1D3557', labelpad=10)

ax.grid(True, linestyle=':', alpha=0.6, color='#cccccc')
ax.legend(loc='upper left', frameon=True, facecolor='#ffffff', edgecolor='#cccccc', prop={'weight': 'bold', 'size': 10.5})

ax.set_title("Normalized 5-Fold Cross-Validation Performance Across 9 Rating Architectures (Model M32)", fontsize=13.5, fontweight='bold', color='#1D3557', pad=14)
