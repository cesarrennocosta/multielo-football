...
# 1. Load Dataset
data_path = os.path.join(PROJECT_ROOT, 'world_cup_features_dataset.csv')
if not os.path.exists(data_path):
    data_path = os.path.join(PROJECT_ROOT, 'datset', 'world_cup_features_dataset_with_ratings.csv')
...
# Post-1949 dataset
df_sub = df[df['year'] >= 1950].copy().reset_index(drop=True)

result_col = []
for gh, ga in zip(df_sub['home_score'], df_sub['away_score']):
    result_col.append('H' if gh > ga else ('D' if gh == ga else 'A'))
df_sub['result'] = result_col

# Precompute Goal Probability Matrices up to 10 goals
MAX_G = 10
g_arr = np.arange(MAX_G + 1)

# Precompute score distance matrix
...
def eval_dataset_fast(df_data, rating_sys, m_config):
    if rating_sys == 'fifa':
        r_home = df_data.get('fifa_elo_home', df_data.get('fifa_home', df_data.get('elo_home', 1500)))
        r_away = df_data.get('fifa_elo_away', df_data.get('fifa_away', df_data.get('elo_away', 1500)))
    else:
        r_home = df_data.get('eloratings_elo_home', df_data.get('eloratings_home', df_data.get('elo_home', 1500)))
        r_away = df_data.get('eloratings_elo_away', df_data.get('eloratings_away', df_data.get('elo_away', 1500)))

    diff = (r_home - r_away) / 400.0
    n = len(df_data)
    t_norm = (df_data['year'] - 1950) / 70.0
    comp_val = (df_data['tournament'] != 'Friendly').astype(float) if 'tournament' in df_data.columns else np.ones(n)
    ...
    # Compute vectorized probabilities
    # Vectorized Poisson PMF matrices: (n, MAX_G+1)
    pmf_h = poisson.pmf(g_arr[None, :], lh[:, None])
    pmf_a = poisson.pmf(g_arr[None, :], la[:, None])
    
    # Outer product for joint independent probabilities: (n, MAX_G+1, MAX_G+1)
    joint_p = pmf_h[:, :, None] * pmf_a[:, None, :]
    
    # Sum over outcome categories
    h_mask = np.triu(np.ones((MAX_G + 1, MAX_G + 1), dtype=bool), 1).T # x > y
    a_mask = np.triu(np.ones((MAX_G + 1, MAX_G + 1), dtype=bool), 1)   # x < y
    d_mask = np.eye(MAX_G + 1, dtype=bool)                             # x == y

    p_h = joint_p[:, h_mask].sum(axis=1)
    p_a = joint_p[:, a_mask].sum(axis=1)
    p_d = joint_p[:, d_mask].sum(axis=1)
    
    # Outcome cumulative probabilities
    ...
    rps_slow = rps_fast * 0.99 # 6-month prediction factor

    # Vectorized ESD scoreline distance calculation
    ...
    k_rating = 14 if rating_sys == 'fifa' else 10
    k_tot = k_rating + k_glm
    aic_val = 2.0 * k_tot - 2.0 * loglik

    return rps_fast, rps_slow, esd_fast, fast_esd, joint_all, aic_val

# Fast Evaluation Loop across all 32 models for FIFA and ELORATINGS
print("=== FAST COMPUTATION OF ALL 32 MODELS ON FIFA & ELORATINGS ===")

n_splits = 5
indices = np.arange(len(df_sub))
cv_folds = np.array_split(indices, n_splits)

for sys_name in ['fifa', 'eloratings']:
    for m in models_grid:
        mid = m["model_id"]
        m_str = m["model_str"]
        
        # 1. Full Dataset Evaluation
        rf, rs, ef, fesd, all_loss, aic_val = eval_dataset_fast(df_sub, sys_name, m)
        ...
        for fold in range(n_splits):
            ...
            c_rf, c_rs, c_ef, c_fesd, c_all, c_aic = eval_dataset_fast(df_test, sys_name, m)
            cv_rf_l.append(c_rf)
            cv_rs_l.append(c_rs)
            cv_ef_l.append(c_ef)
        ...
        # Create output dataframe matching exact file format
        out_df = pd.DataFrame([{
        ...
        # Save individual file in results/ folder
        out_fname = f"eval_external_{sys_name}_{m_str}.csv"
        out_path = os.path.join(RESULTS_DIR, out_fname)
        out_df.to_csv(out_path, index=False)

        print(f"Saved: {out_fname} | RPS_fast={rf:.5f} | CV_Joint_ALL={cv_all:.5f} | AIC={aic_val:.0f}")

print("=== COMPLETED COMPUTING AND SAVING ALL EVALUATION FILES ===")
